...
<html lang="en">
  <head>
    ...
    <link rel="stylesheet" type="text/css" href="./css/dayprofitfront.css" />
...
$sql = "SELECT COUNT(ticket_no) AS sum_ticket_no, MAX(trip_no) AS sum_trip_no, SUM(adult) AS sum_adult, SUM(child) AS sum_child, SUM(luggage) AS sum_luggage, SUM(price) AS sum_price FROM ticketdata WHERE date = CURDATE();";
...
if ($result->num_rows > 0) {
  ...
  <table id="tablet">
        ...
        $sql = "SELECT ticket_no, trip_no, date, time, starting_point, ending_point, adult, child, luggage, price FROM ticketdata WHERE date = CURDATE();";
        $result = $conn->query($sql);
